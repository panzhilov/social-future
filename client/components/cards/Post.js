              ...
              {post.comments.slice(0, commentCount).map((comment) => (
                <li key={comment._id} className="list-group-item d-flex justify-content-between align-items-start">
                  <div className="ms-2 me-auto">
                    <div>
                    {comment && comment.author && comment.author.name && (<Avatar size={30} className="mb-1 mr-3">
                  ...
                  <span className="badge rounded-pill text-muted">
                    ...
                    {state && state.user && state.user._id === comment.author._id && (
                      <div className="ml-auto mt-1">
                        <DeleteOutlined onClick={() => removeComment(post._id, comment)} className="pl-3 text-danger" />
                      </div>
                    )}
                  </span>
                </li>
